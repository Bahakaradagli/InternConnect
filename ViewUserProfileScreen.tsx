...
interface UserInfo {
  ...
  personalInfo?: { profileImage?: string; profileImageTwo?: string };
}
...
export default function ViewUserProfileScreen({ route }) {
  ...
  useEffect(() => {
    ...
    onValue(collectionsRef, (snapshot) => {
      ...
      if (data) {
        setCollections(data);
        setSelectedCollection(Object.keys(data)[0] as any); // ilk başlığı otomatik seç
      }
    });
  }, [userId]);
  ...
  useEffect(() => {
    ...
    onValue(typeRef, (snapshot) => {
      ...
      if (type === 'admin') {
        navigation.reset({
          index: 0,
          routes: [{ name: 'AdminSide' as never }],
        });
      }
    });
  }, []);
  ...
  const getGroupedSnapsFromCollection = () => {
    ...
    Object.entries(snapsByDate).forEach(([date, snaps]) => {
      if (snaps && typeof snaps === 'object') {
        Object.values(snaps as Record<string, any>).forEach((snap: any) => {
          allSnaps.push({ ...(typeof snap === 'object' ? snap : {}), date });
        });
      }
    });
  ...
  const getSnapsToRender = () => {
    ...
    Object.entries(snapData).forEach(([snapId, snap]) => {
      if (snap && typeof snap === 'object' && 'imageUrl' in snap) {
        snapList.push({ ...(snap as any), date: new Date((snap as any).timestamp).toISOString().slice(0, 10) });
      }
    });
  ...
  useEffect(() => {
    ...
    onValue(userAppliedRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        const jobIds = Object.values(data as any[])
          .filter((job: any) => job.companyId === userId)
    ...
    onValue(ref(db, `users/${userId}/snaps`), (snapshot) => {
      ...
      if (data) {
        Object.entries(data).forEach(([date, snaps]) => {
          if (snaps && typeof snaps === 'object') {
            Object.values(snaps as Record<string, any>).forEach((snap: any) => {
              snapList.push({ ...(typeof snap === 'object' ? snap : {}), date });
            });
          }
        });
      }
      setSnaps(snapList.reverse() as any[]);
    });
  }, [userId]);
